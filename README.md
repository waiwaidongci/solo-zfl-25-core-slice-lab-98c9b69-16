...
- 健壮性：逐分块 CRC 校验；结构校验拒绝缺少 IEND、IEND 含内容、IEND 后残留分块或数据、重复 IHDR、未知关键分块、IDAT 被其他分块隔开、调色板重复或晚于图像数据、透明信息重复等损坏文件；内容校验拒绝非法分块类型（非 4 字母或保留位错误）、调色板长度非 3 倍数或超上限、透明信息晚于图像数据或与颜色类型不符（连续多 IDAT、自定义辅助分块、建议调色板、合规透明信息等合法形式不受影响），损坏图像一律不留统计；分析名称等用户输入在页面一律转义为纯文本；图像原子写入，记录保存失败时自动清理孤立文件且已有结果不变。
